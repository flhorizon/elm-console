module Console.Runner exposing (..)
...
run io =
    let
        ...
        third ( _, _, z ) =
            z
    in
        NativeCom.sendRequests (Signal.map third <| foldp f init NativeCom.responses)
...
flattenReqs rs =
    ...
    in
        Trampoline.trampoline <| loop rs [] 0
    ...
    let
        newST =
            case resp of
                ...
                Just s ->
                    { buffer = String.append st.buffer s }

        ( newST', ( rs, k ) ) =
            extractRequests (io ()) newST
    in
        ( k, newST', rs )
